...
DrawObject::DrawObject(unique_ptr<sf::Drawable> spriten)
    : sprite(std::move(spriten)) {}

DrawObject::DrawObject(unique_ptr<sf::Drawable> spriten, bool shown)
...
TextObject::TextObject(const string& str, size_t size, const Color& color,
                       ...
                       bool show)
    : DrawObject(std::make_unique<Text>(str, font, size), show) {
...
RectObject::RectObject(const Vector2f& size, const Color& color,
    ...
    : DrawObject(std::make_unique<RectangleShape>(size), show) {
